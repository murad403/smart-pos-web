...
import { ShoppingCart, Menu, LogOut, PackageSearch, DollarSign, Settings, ChefHat } from "lucide-react";
        ...
        <>
                    ...
                    <div className="hidden sm:flex items-center gap-1">
                        {navLinks.map(({ href, icon, label }) => (
                            <Link
                                key={href}
                                href={href}
                                className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${pathname === href
            ...
            <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-[0_-1px_6px_rgba(0,0,0,0.06)]">
                <div className="flex items-center justify-around px-2 py-2">
                    {navLinks.map(({ href, icon, shortLabel }) => (
                        <Link
                            key={href}
                            href={href}
                            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-xl text-[11px] font-medium transition-all ${pathname === href
                                ? "text-blue-600"
                                : "text-gray-500 hover:text-gray-800"
                                }`}
                        >
                            <span
                                className={`p-1.5 rounded-lg transition-all ${pathname === href ? "bg-blue-50" : ""
                                    }`}
                            >
                                {icon}
                            </span>
